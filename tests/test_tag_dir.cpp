...
TEST_CASE(write_tags) {
	...
	EXPECT(entry->tags.at(0).value == tags_to_write[0]);
	EXPECT(entry->tags.at(1).value == tags_to_write[1]);
}
...
TEST_CASE(append_tags) {
	...
	EXPECT(entry->tags.at(0).value == tags_to_write[0]);
	EXPECT(entry->tags.at(1).value == tags_to_write[1]);
	EXPECT(entry->tags.at(2).value == tags_to_append[0]);
	EXPECT(entry->tags.at(3).value == tags_to_append[1]);
}
